...
- Meetings: add or edit entries under `meetings:` — each has `label`, `name`, an optional `description` (overview), `dates`, a venue address as `location_lines` (a list of lines, shown one per line; a plain `location` string also works), an optional `host` (with `text` and an optional `url`, shown as a link), `contact`, an optional `agenda` (a list of items with `day` and `title`, shown as a day-by-day schedule), an optional `pre_meeting` (a short note shown below the agenda), and an optional `note`. To add a future meeting, copy the whole block of the first one and edit the values.
